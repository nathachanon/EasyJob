...
namespace EasyJob
{
    ...
    public partial class GetJobSuccess : ContentPage
    {
        private readonly HttpClient _client = new HttpClient();
        private const string Url = "http://139.180.129.212/api/work_getfinish";
        private ObservableCollection<Job> my_work_get_finish;
        public GetJobSuccess()
        {
            InitializeComponent();
        }
            ...
            using (HttpResponseMessage response = await _client.PostAsync(Url, content))
            {
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    using (HttpContent contents = response.Content)
                    {
                        ...
                        my_work_get_finish = new ObservableCollection<Job>(work_list);
                        ItemlistView.ItemsSource = my_work_get_finish;
